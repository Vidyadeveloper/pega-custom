...
export function getAllFields(pConnect: () => any) {
  ...
  const hasRegions = metadata.children.length >= 4; // Ensure detection of four regions
...
export function getFilteredFields(getPConnect: Function) {
  ...
  let tertiaryFieldsRaw; // NEW third layout region
  let quaternaryFieldsRaw;

  const metadata = getPConnect().getRawMetadata();
  // const hasRegions = !!metadata.children[0]?.children;
  const hasRegions = metadata.children.length >= 3; // Ensure detection of three regions
  if (hasRegions) {
    primaryFieldsRaw = metadata.children[0].children;
    secondaryFieldsRaw = metadata.children[1].children;
    tertiaryFieldsRaw = metadata.children[2].children;
    quaternaryFieldsRaw = metadata.children[3].children || []; // Adding fourth region
  } else {
  ...
  tertiaryFieldsRaw = tertiaryFieldsRaw?.filter((item: any) => {
    const resolvedItem = getPConnect().resolveConfigProps(item.config);
    return resolvedItem.visibility !== false && item.type !== 'TextContent';
  });
  quaternaryFieldsRaw = quaternaryFieldsRaw?.filter((item: any) => {
    const resolvedItem = getPConnect().resolveConfigProps(item.config);
    return resolvedItem.visibility !== false && item.type !== 'TextContent';
  });

  return [primaryFieldsRaw, secondaryFieldsRaw, tertiaryFieldsRaw, quaternaryFieldsRaw];
}
 ...
 */
export function getLayoutDataFromRegion(regionData: any) {
...
export function getIsAssignmentView(pConnect: Function) {
  ...
  const assignmentViewName = pConnect().getValue(
    PCore.getConstants().CASE_INFO.ASSIGNMENTACTION_ID
  );
 ...
 */
// eslint-disable-next-line @typescript-eslint/ban-types
export function getInstructions(pConnect: Function, instructions = 'casestep') {
